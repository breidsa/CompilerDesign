...
/* ----------------- Global Hashmaps and Main Function ---------------- */
 ...
 static void existsID(Yytoken name){
     Object val = name.getValue();
     if(symbolTable.find_symbol(val) != null){
       System.out.println("ERROR SYMBOL EXISTS"); 
     }
 }
}
...
%%  
    ...
    struct : STRUCT IDENTIFIER LBRACKET declarationList RBRACKET { $$ = new StructCreator($2, $4); Struct st = new Struct($2.getValue(), (StmtList)$4); symbolTable.addScope(); symbolTable.add_symbol(st);}
    ;      
    ...
    declaration: type IDENTIFIER {$$ = new VarDef($1, $2); System.out.println("MY VALUE IS    " + ($2).getValue()); Var addMe = new Var(($2).getValue(), $1); symbolTable.add_symbol(addMe); }
    ;
    ...
    | IDENTIFIER EQ exp SEMICOLON { $$ = new Asnmt($1, $3);}// existsID($1); }
...
/* ------------------------------------------------------------------------ */
  ...
  class SymbolTable{
   ...
   public ID check_scope(ID id){
      if(this.table.get(this.scope-1).containsKey(id.getName())){
